...
- Evolución mensual del consumo para los años 
- Evolución diaria del consumo para los meses de Enero y Agosto
- Evolución semanal del consumo medio diario (también del total, pero sin mucho interés para el cliente)
- Evolución diaria del consumo los Lunes
- Evolución semanal del consumo

**Nota:** Se puede ocultar todo el código en la esquina superior derecha, y así solo mostrar el código que sea pertinente.
...
colnames(Comparacion) <- c("Submedidor","RMSE Sin Estacionalidad","RMSE Con estacionalidad")
...
PrediccionFinalCocinaAnual <- forecast(tsSM1_HW_Mensual, h=25)
## Plot only the forecasted area
...
```{r}
 # Pronóstico para los siguientes 25 meses 
 PrediccionFinalLavaderoAnual <- forecast(tsSM2_HW_Mensual, h=25)
 ## Plot only the forecasted area
...
```{r}
# Pronóstico para los siguientes 26 meses 
PrediccionFinalAcTAnual <- forecast(tsSM3_HW_Mensual, h=25)
## Plot only the forecasted area
...
df<-data.frame(
  Fecha=rep(c("2010-12-01"	,"2011-1-01"	,"2011-2-01"	,"2011-3-01"	,"2011-4-01"	,"2011-5-01"	,"2011-6-01"	,"2011-7-01"	,"2011-8-01"	,"2011-9-01","2011-10-01"	,"2011-11-01"	,"2011-12-01"),3) ,
           Consumo=c(as.vector(  PrediccionFinalCocinaAnual[["mean"]][13:25]),
                     as.vector(PrediccionFinalLavaderoAnual[["mean"]][13:25]),
                     as.vector(     PrediccionFinalAcTAnual[["mean"]][13:25])
                     )   ,
...
ggplot(df,aes(x = as.character(Fecha), y = Consumo, group=SM,colour=SM)) +
          ...
          theme_minimal()+
  labs(
    title = "Pronóstico del consumo energético mensual para el año 2011",
...
dfPlotlyPronosticoMensual$Cocina <- as.vector(  PrediccionFinalCocinaAnual[["mean"]][13:25])
dfPlotlyPronosticoMensual$Lavadero<-as.vector(PrediccionFinalLavaderoAnual[["mean"]][13:25])
dfPlotlyPronosticoMensual$AireTermo<-as.vector(    PrediccionFinalAcTAnual[["mean"]][13:25])


dfPlotlyPronosticoMensual$Lavadero <- ifelse(
  dfPlotlyPronosticoMensual$Lavadero < 0 , 0 ,   dfPlotlyPronosticoMensual$Lavadero 
)
...
#### Evolución mensual del consumo (sin estacionalidad)
...
- Segunda interacción: tendencia del consumo
...
## Plot only the forecasted area
plot(tsSM3_HW_DiariaEnero_forC, ylab= "Vatios-Hora", xlab="Fecha - AireAcondicionado y termo eléctrico", main="Predicción para el mes de Enero del año 2010", start(2010))
```
...
```{r}
# Pronóstico para los siguientes 13 meses 
PrediccionFinalCocinaEnero <- forecast(tsSM1_HW_DiariaEnero, h=62)
## Plot only the forecasted area
...
```{r}
# Pronóstico para los siguientes 13 meses 
PrediccionFinalLavaderoEnero <- forecast(tsSM2_HW_DiariaEnero , h=62)
## Plot only the forecasted area
...
Predicciones negativas, no son fiables.
...
```{r}
# Pronóstico para los siguientes 13 meses 
PrediccionFinalACTermoEnero <- forecast(tsSM3_HW_DiariaEnero , h=62)
## Plot only the forecasted area
...
```{r}
Fecha=c("2011-01-1"  ,  "2011-01-2"    ,"2011-01-3"    ,"2011-01-4"    ,"2011-01-5"        ,"2011-01-6"    ,"2011-01-7"    ,"2011-01-8"    ,"2011-01-9"    ,"2011-01-10","2011-01-11" ,"2011-01-12"   ,"2011-01-13","2011-01-14"    ,"2011-01-15"   , "2011-01-16"   , "2011-01-17"   , "2011-01-18"   , "2011-01-19"   , "2011-01-20", "2011-01-21" , "2011-01-22"   , "2011-01-23"   , "2011-01-24"   , "2011-01-25"   , "2011-01-26"   , "2011-01-27"   , "2011-01-28"   , "2011-01-29"   , "2011-01-30", "2011-01-31"  ) 

Fecha<- as.Date(Fecha, format = "%Y-%m-%d")

dfPlotlyPronosticoEnero<-data.frame( Fecha=Fecha )

dfPlotlyPronosticoEnero$Cocina<-as.vector(    PrediccionFinalCocinaEnero[["mean"]][32:62] )
dfPlotlyPronosticoEnero$Lavadero<-as.vector(PrediccionFinalLavaderoEnero[["mean"]][32:62] )
dfPlotlyPronosticoEnero$AireTermo<-as.vector(PrediccionFinalACTermoEnero[["mean"]][32:62] )


dfPlotlyPronosticoEnero$Cocina<- 
ifelse(dfPlotlyPronosticoEnero$Cocina < 0 , 0 , dfPlotlyPronosticoEnero$Cocina)

 save(dfPlotlyPronosticoEnero, file =   "Datos/Pronostico/DF_Energia_PronosticoEnero_11.RData")
```
...
```{r}
tsSM2_HW_DiariaAgosto<- HoltWinters(tsSM2_DiariaAgosto_detrended , gamma = TRUE,
                                    alpha=TRUE,
                                    beta=TRUE
                                   )
...
```{r}
# Pronóstico para los siguientes 13 meses 
PrediccionFinalCocinaAgosto <- forecast(tsSM1_HW_DiariaAgosto, h=62)
## Plot only the forecasted area
...
```{r}
 # Pronóstico para los siguientes 13 meses 
 PrediccionFinalLavaderoAgosto <- forecast(tsSM2_HW_DiariaAgosto, h=62)
 ## Plot only the forecasted area
 plot(PrediccionFinalLavaderoAgosto, ylab= "Vatios-Hora", xlab="Fecha", main="Predicción el consumo energético \n  del # lavadero para Agosto del 2011", start(2011))
```
...
No obtenemos buenas predicciones para el lavadero.
...
```{r}
 # Pronóstico para los siguientes 13 meses 
 PrediccionFinalTermoAcAgosto <- forecast(tsSM3_HW_DiariaAgosto, h=62)
 ## Plot only the forecasted area
...
dfPlotlyPronosticoAgosto<-data.frame(
  Fecha=Fecha )
dfPlotlyPronosticoAgosto$Cocina <- as.vector(  PrediccionFinalCocinaAgosto[["mean"]][32:62] )
dfPlotlyPronosticoAgosto$Lavadero<-as.vector(PrediccionFinalLavaderoAgosto[["mean"]][32:62])
dfPlotlyPronosticoAgosto$AireTermo<-as.vector(PrediccionFinalTermoAcAgosto[["mean"]][32:62])
...
# Evolución media del consumo según día de la semana
...
```{r}
Granularidad_semanasMedia <- Granularidad_horas %>% group_by(year, weekday) %>% 
   summarize(
      Sub_metering_1 = mean(Sub_metering_1),
      Sub_metering_2 = mean(Sub_metering_2),
      Sub_metering_3 = mean(Sub_metering_3)
   )
seriesSemanalesMedia <- rbind.data.frame(
   filter(Granularidad_semanasMedia, year== 2007  ),
   filter(Granularidad_semanasMedia, year== 2008  ),
   filter(Granularidad_semanasMedia, year== 2009  )
)

## Creamos un objeto TS
 tsSM1_Semanal<- ts(seriesSemanalesMedia$Sub_metering_1,frequency=7, start=c(2007,1))
 tsSM2_Semanal<- ts(seriesSemanalesMedia$Sub_metering_2,frequency=7, start=c(2007,1))
 tsSM3_Semanal<- ts(seriesSemanalesMedia$Sub_metering_3,frequency=7, start=c(2007,1))
```
...
Parece que la componente irregular no sigue ningún patrón determinado, además, hemos podido eliminar la componente estacioal, ya que se mueve en unos rangos muy pequeños, del orden de $10^{-8}$
...
plot(tsSM1_HW_Semanal,
     ...
     main="Suavizado exponencial de Holt-Winters")
```
El pronóstico de consumo de la cocina variará con un pronóstico de 46 vatios hora como mínimo y 84   como máximo.
...
Hemos vuelto a descomponer la serie y hemos logrado eliminar la estacionalidad, ya que se mueve en unos órdenes muy peuqeños.
Tendenciea decreciente del consumo. 
...
El pronóstico de consumo energético para el lavadero es de 5 a 130  vatios-hora.
...
plot(tsSM2_HW_Semanal,
     ...
     main="Suavizado exponencial de Holt-Winters")
```
Serie muy afectada por valores extremos, pero se aprecia una tendencia creciente.
...
Hemos conseguido eliminar la estacionalidad
...
Predicción del consumo para el termo eléctrico y aire acondicionado entre 347 vatios hora.
...
```{r}
Granularidad_semanasMediaReal <- Granularidad_horas %>% group_by(year, weekday) %>% 
   summarize(
      Sub_metering_1 = mean(Sub_metering_1),
      Sub_metering_2 = mean(Sub_metering_2),
      Sub_metering_3 = mean(Sub_metering_3)
...
```{r}
RMSE_SM1_GranSemanalFor<-accuracy(tsSM1_HW_Semanal_forC   ,Granularidad_semanasMediaReal$Sub_metering_1)[4]
RMSE_SM2_GranSemanalFor<-accuracy(tsSM2_HW_Semanal_forC   ,Granularidad_semanasMediaReal$Sub_metering_2)[4]
RMSE_SM3_GranSemanalFor<-accuracy(tsSM3_HW_Semanal_forC   ,Granularidad_semanasMediaReal$Sub_metering_3)[4]
...
El error es bastante bajo, se trata de un gran modelo para el termo eléctrico y aire y para el lavadero, teniendo en cuenta el consumo medio diario de estos dispositivos, sin embargo, para la cocina no es buen modelo, ya que el consu
...
```{r}
# Pronóstico para los siguientes 13 meses 
PrediccionFinalCocinaSemanal<-forecast(tsSM1_HW_Semanal, h=14, level = c(95))
## Plot only the forecasted area
plot(PrediccionFinalCocinaSemanal, 
     ylab= "Vatios-Hora", xlab="Fecha", 
     main="Predicción el consumo energético medio de la cocina \n según día de para año 2011", start(2010),ylim=c(40,100))
```
...
```{r}
# Pronóstico para los siguientes 13 meses 
PrediccionFinalLavaderoSemanal <- forecast(tsSM2_HW_Semanal, h=14)
## Plot only the forecasted area
plot(PrediccionFinalLavaderoSemanal, ylab= "Vatios-Hora", xlab="Fecha", main="Predicción el consumo energético del lavadero \n  Según el día de la semana para los próximos 12 meses", start(2010))
```
...
```{r}
# Pronóstico para los siguientes 13 meses 
PrediccionFinalAcTSemanal <- forecast(tsSM3_HW_Semanal, h=14)
## Plot only the forecasted area
plot(PrediccionFinalAcTSemanal, ylab= "Vatios-Hora", xlab="Fecha", main="Predicción el consumo energético del aire y termo \n para cada día de la semana", start(2011),ylim=c(350,500))
```

Las predicciones son las siguientes: 

```{r}
PrediccionFinalAcTSemanal[["mean"]] 
```
...
### Representación y exportación de datos para el cliente


```{r}
dfSemanalMedio<- data.frame(
  Fecha=1:7
)
dfSemanalMedio$Cocina<-as.vector(PrediccionFinalCocinaSemanal[["mean"]] )[8:14]
dfSemanalMedio$Lavadero<-as.vector(PrediccionFinalLavaderoSemanal[["mean"]] )[8:14]
dfSemanalMedio$AireTermo<-as.vector(PrediccionFinalAcTSemanal[["mean"]] )[8:14]


 save(dfSemanalMedio, file = "Datos/Pronostico/DF_Energia_PronosticoSemanal_11_Medio.RData")
```
...
## Pronóstico del consumo energétido del Lavadero para los Lunes del año 2011
...
tsSM3_HW_Semanal_forC <- forecast(tsSM3_HW_Semanal, h=7, level=c(95))
## Plot only the forecasted area
plot(tsSM3_HW_Semanal_forC, ylab= "Vatios-Hora", xlab="Fecha - AireAcondicionado y termo eléctrico", main="Predicción semanal del consumo energético para el año 2010", start(2010), ylim=c(470000,550000))
```
...
Hemos conseguido unos modelos con unos errores aceptables, ya que antes obteníamos los siguientes errores:
...
seriesSemanalesCompletas <- Granularidad_semanas %>% filter(year != 2006)
...
plot(PrediccionFinalCocinaSemanal, 
     ylab= "Vatios-Hora", xlab="Fecha", 
     main="Predicción el consumo energético de la cocina \n según día de para año 2011", start(2011), ylim = c(4000,40000))
```
...
## Plot only the forecasted area
plot(PrediccionFinalLavaderoSemanal, ylab= "Vatios-Hora", xlab="Fecha", main="Predicción el consumo energético del lavadero \n  Según el día de la semana para los próximos 12 meses", start(2011), ylim = c(20000,110000))
```
...
```{r}
# Pronóstico para los siguientes 13 meses 
PrediccionFinalAcTSemanal <- forecast(tsSM3_HW_Semanal, h=7)
## Plot only the forecasted area
plot(PrediccionFinalAcTSemanal, ylab= "Vatios-Hora", xlab="Fecha", main="Predicción el consumo energético del aire y termo \n para cada día de la semana", start(2011),ylim=c(420000,480000))
```
...
```{r}
dfSemanal<- data.frame(
  Fecha=1:7
)
dfSemanal$Cocina<-as.vector(PrediccionFinalCocinaSemanal[["mean"]] )
dfSemanal$Lavadero<-as.vector(PrediccionFinalLavaderoSemanal[["mean"]] )
dfSemanal$AireTermo<-as.vector(PrediccionFinalAcTSemanal[["mean"]] )

# save(dfSemanal, file = "Datos/Pronostico/DF_Energia_PronosticoSemanal_11.RData")
```
...
plot_ly(dfSemanal,
       ...
       y = ~Cocina,
             name = "Cocina" , type='scatter',  mode='lines'  ) %>% 
    add_trace( y = ~dfSemanal$Lavadero,name = "Lavadero" , mode='lines')%>%
    add_trace( y = ~dfSemanal$AireTermo, name = "Aire acondicionado y termo" , mode='lines') %>%
    layout(title = "Pronóstico del consumo energético según día de la semana para el año 2011",
           xaxis = list(title = "Día de la semana",
                        ticktext=c("Lunes","Martes","Miércoles","Jueves","Viernes","Sábado","Domingo"),
                        tickvals=dfSemanal$Fecha)
           , yaxis = list (title = "Energía (Varios-hora)"))
```


**Nota**: El consumo representado es el consumo total para cada día de la semana del año 2011. Por ejemplo, el dato correspondiente al lunes será la energía total consumida por todos los lunes de ese año, y por tanto no tiene sentido su representación al cliente, ya que se le ha ofrecido la energía media de cada día de la semana.
...
Remainder no es un muelle. Observamos tendencia creciente y fuerte componente estacional, se trata de la componente con mas peso.




```{r}
Componentes_SM1_SemanalSTL<- tsSM1_Semanal %>%stl( t.window=53,
  s.window="periodic",  robust=TRUE)
autoplot(Componentes_SM1_SemanalSTL, ts.colour = 'blue')
```

Se aprecia tendencia decreciente del consumo
...
```{r}
Componentes_SM2_SemanalSTL<- tsSM2_Semanal %>%stl( t.window=53,
  s.window="periodic",  robust=TRUE)
autoplot(Componentes_SM2_SemanalSTL, ts.colour = 'blue')
```

Al igual que para la cocina, se parecia una tendencia de consumo decreciente, más acentuada durante el año 2008. Sin embargo, en el año 2009 se puede ver un pequeño repunte en el consumo energético.
...
El peso de la componente estacional es bastante alto.


```{r}
Componentes_SM3_SemanalSTL<- tsSM3_Semanal %>%stl( t.window=53,
  s.window="periodic",  robust=TRUE)
autoplot(Componentes_SM3_SemanalSTL, ts.colour = 'blue')
```

La tendencia es creciente, aunque hay períodos en los que hubo decrecimiento del consumo energético, como la primera mitad del año 2007 y la segunda del año 2008.
...
plot.ts(tsSM1_Semanal_detrended, 
        ylab = "Energía (Vatios-hora)", 
        main = 'Variabilidad del consumo energético. Sin estacionalidad',
        cex.main = 0.85)
```
Vamos a estudiar los resíduos
...
La componente irregular no sigue ningún patrón definido.
...
- Primera iteracción: rango de predicción
...
Las predicciones se moverán en un rango de 0 a 30 mil vatios-hora de consumo energético semanal.
...
plot.ts(tsSM2_Semanal_detrended, 
        ylab = "Energía (Vatios-hora)", 
        main = 'Variabilidad del consumo energético. Serie sin estacionalidad',
        cex.main = 0.85)
```
...
VAmos a volver a descomponer la serie
...
Las predicciones se moverán en un rango de 0 a 28 mil vatios-hora de consumo energético semanal.
...
plot(tsSM2_HW_Semanal,
     ...
     main="Suavizado exponencial de Holt-Winters")
```
Tendencia creciente  al consumo, sobre todo en el año 2009.
...
## Plot only the forecasted area
plot(tsSM2_HW_Semanal_forC, ylab= "Vatios-Hora", xlab="Fecha - AireAcondicionado y termo eléctrico", main="Predicción semanal para el año 2010", start(2010),ylim=c(0,50000))
```
...
plot.ts(tsSM3_Semanal_detrended, 
        ylab = "Energía (Vatios-hora)", 
        main = 'Variabilidad del consumo energético',
        cex.main = 0.85)
```
...
Las predicciones se moverán en un rango de 20 a 90 mil vatios-hora de consumo energético semanal.
...
Tendencia creciente  al consumo.
...
## Plot only the forecasted area
plot(tsSM3_HW_Semanal_forC, ylab= "Vatios-Hora", xlab="Fecha - AireAcondicionado y termo eléctrico", main="Predicción semanal del consumo energético para el año 2010", start(2010),ylim=c(40000,150000))
```
...
ResEvolSemanal2 %>% kable(booktabs=TRUE) %>% kable_styling(latex_options = "striped")
```
...
#### Pronóstico del consumo para cada semana del año 2011

```{r}
GS <- Granularidad_semanas %>% filter(year != 2006)
## Creamos un objeto TS
tsSM1_Semanal<- ts(GS$Sub_metering_1,         frequency=53, start=c(2007,1))
tsSM2_Semanal<- ts(GS$Sub_metering_2,         frequency=53, start=c(2007,1))
tsSM3_Semanal<- ts(GS$Sub_metering_3,         frequency=53, start=c(2007,1))
```
...
tsSM1_Semanal_detrended <- tsSM1_Semanal - Componentes_SM1_SemanalSTL$time.series[,1]
# Suavizado tal y como obtenemos el mejor modelo
## Holt Winters Exponential Smoothing & Plot
tsSM1_HW_Semanal <- HoltWinters(tsSM1_Semanal_detrended ,
                             alpha=TRUE,
                          #   beta=TRUE,
...
```{r}
# Pronóstico para las próximas 53 semanas
PrediccionFinalCocinaSemanal <- forecast(tsSM1_HW_Semanal, h=53)
## Plot only the forecasted area
plot(PrediccionFinalCocinaSemanal, ylab= "Vatios-Hora", xlab="Fecha", main="Predicción el consumo energético \n  de la cocina para el año 2011", start(2011),ylim = c(0,25000))
```
...
Hay muchas predicciones negativas
...
```{r}
# Pronóstico para las próximas 53 semanas
PrediccionFinalLavaderoSemanal <- forecast(tsSM2_HW_Semanal, h=53)
## Plot only the forecasted area
plot(PrediccionFinalLavaderoSemanal, ylab= "Vatios-Hora", xlab="Fecha", main="Predicción el consumo energético \n  del lavadero para el año 2011", start(2011))
```
...
```{r}
# Pronóstico para las próximas 53 semanas
PrediccionFinalACyTSemanal <- forecast(tsSM3_HW_Semanal, h=53)
## Plot only the forecasted area
plot(PrediccionFinalACyTSemanal, ylab= "Vatios-Hora", xlab="Fecha", main="Predicción el consumo energético \n  del lavadero para el año 2011", start(2011))
```

Las predicciones son las siguientes: 

```{r}
PrediccionFinalACyTSemanal[["mean"]]
```


Predicciones negativas la mayoría
...
No tenemos buenas predicciones, por lo que no tiene sentido ofrecerlas al cliente.
...
## Pronóstico mensual del consumo energético del año 2011

Se observó una tendencia creciente del consumo de la cocina y del aire acondicionado y termo eléctrico, sin embargo, para el lavadero la tendencia ha sido decreciente.

Los valores extremos tienen gran influencia a la hora de pronosticar el consumo energético

## Pronóstico del consumo energético diario para los meses de Agosto y Enero del año 2011

Para el mes de enero la tendencia del consumo es creciente par a todos los aparatos submeterizados, sin embargo, en la primera mitad del mes de enero del año 2008 hay un desdenso brusco del consumo energético, pero posteriormente se vuelve a la tendencia habitual de consumo creciente.

En los tres casos, la componente estacional tiene gran peso y es muy influyente, los datos estan afectados por la estacionalidad.
Por ejemplo, para la cocina y el aire acondicionado y termo eléctrico, el consumo es mayor en la segunda semana del mes de Enero, y esto se repite cada año.

Para el mes de Agosto, hay un patrón que se cumple en las tres zonas de submedición, y es que durante el año 2008, el consumo energético fué prácticamente nulo.

Para estos datos, tiene gran peso la tendencia del consumo. Las predicciones podrán verse afectadas por este año en el que el consumo fué drásticamente bajo.

## Pronñostico del consumo semanal medio para cada día de la semana del año 2011

El El gráfico de evolución del consumo energético medio para cada día de la semana durante el período 2007-2009 nos muestra
que el consumo fué más elevado para los fines de semana. Es interesante recalcar que para el aire acondicionado y termo eléctrico el consumo creción bruscamente cada año, sin embargo, para el resto de electrodomésticos en la cocina y lavadero, la tendencia del consumo fué decreciente respecto al tiempo.